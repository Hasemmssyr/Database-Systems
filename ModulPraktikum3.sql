...
-- number 5
SELECT customer.cst_name, customer.cst_phoneNumber, membership.mbr_email, COUNT(transaction.tr_id) AS totalTransaction
FROM customer
JOIN membership ON customer.cst_id = membership.customer_cst_id
JOIN transaction ON customer.cst_id = transaction.customer_cst_id
GROUP BY customer.cst_name, customer.cst_phoneNumber, membership.mbr_email
ORDER BY totalTransaction DESC
LIMIT 3;

-- number 6
WITH cte AS (
    SELECT AVG(tr_totalbill) AS average
    FROM transaction
)
SELECT customer_cst_id, ROUND(AVG(tr_totalbill), 2) AS billaverage
FROM transaction
GROUP BY customer_cst_id
HAVING billaverage > (SELECT average FROM cte)
ORDER BY billaverage DESC;

-- number 7
WITH cte AS (
    SELECT employee_emp_id AS employeeid, COUNT(employee_emp_id) AS totaltransactions
    FROM transaction
    GROUP BY employee_emp_id
)
SELECT cte.employeeid, employee.emp_name, cte.totaltransactions
FROM employee
JOIN cte ON employee.emp_id = cte.employeeid
ORDER BY totaltransactions DESC
LIMIT 1;

-- number 8
WITH cte AS (
    SELECT customer_cst_id, SUM(tr_totalbill) AS totalbill
    FROM transaction
    GROUP BY customer_cst_id
)
